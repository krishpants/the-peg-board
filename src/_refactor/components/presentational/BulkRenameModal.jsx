...
const BulkRenameModal = ({ isOpen, players, onSave, onCancel }) => {
  const [names, setNames] = useState({});
  const firstInputRef = useRef(null);

  // Array of fake testing names with variety
  const fakeNames = [
    "Jamela K", "🔥 FLAMO", "Billy 2", "KC_2239", "Peter", "Victor (D)",
    "Sarah Chen", "Mike 🎾", "Alex Pro", "Jordan_99", "Emma W", "Ryan (C)",
    "Lisa Park", "Tom Swift", "Nina 💪", "Dave_123", "Sophie R", "Chris Lee",
    "Maya (B)", "Jake ⭐", "Zara Khan", "Oliver Q", "Lucy_88", "Sam (A)",
    "Rachel Z", "Max Power", "Ava Singh", "Leo 🏸", "Grace Liu", "Ben_007",
    "Kyle XL", "Tina 🌟", "Hassan M", "Jenny_42", "Rick (S)", "Diana Pro",
    "Felix 🚀", "Amy Zhang", "Carlos_99", "Mia (K)", "Nathan!", "Priya 💫",
    "Oscar.B", "Ruby Chen", "Ian_2024", "Bella (J)", "Marcus 🎯", "Zoe Park",
    "Dylan_X", "Eva Miller", "Raj Kumar", "Luna 🌙", "Tony_888"
  ];

  useEffect(() => {
    if (isOpen && players) {
      // Initialize with current player names
      const initialNames = {};
      Object.values(players).forEach(player => {
        initialNames[player.playerNumber] = player.name || '';
      });
  ...
  const handleFillWithFakeNames = () => {
    ...
    const sortedPlayersList = Object.values(players).sort((a, b) => a.playerNumber - b.playerNumber);
  ...
  const handleSubmit = (e) => {
    ...
    Object.entries(names).forEach(([playerNumber, name]) => {
      ...
      updates[playerNumber] = trimmedName || `Player ${playerNumber}`;
    });
    onSave(updates);
  };
      ...
      <div className="modal modal--bulk-rename">
        ...
        <form onSubmit={handleSubmit} className="modal__form">
          ...
          <div className="modal__actions">
            <button type="button" onClick={onCancel} className="modal__btn modal__btn--cancel">
              Cancel
            </button>
